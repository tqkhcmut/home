...
// tcp data. Returns the position at which the string after
// this string could be filled.
uint16_t fill_tcp_data_v2(uint8_t *buf,uint16_t pos, uint8_t *s)
{
    // fill in tcp data at position pos
    //
    // with no options the data starts after the checksum + 2 more bytes (urgent ptr)
...
//    uint8_t tseq;
	...
	if (next_ack_num)
		seq += next_ack_num;
	// seq = ack
	buf[TCP_SEQ_P] = 	(ack&0xff000000) >> 24;
	buf[TCP_SEQ_P+1] = 	(ack&0x00ff0000) >> 16;
	buf[TCP_SEQ_P+2] = 	(ack&0x0000ff00) >> 8;
	buf[TCP_SEQ_P+3] = 	(ack&0x000000ff);
	// ack = seq
	buf[TCP_SEQACK_P] = 	(seq&0xff000000) >> 24;
	buf[TCP_SEQACK_P+1] = 	(seq&0x00ff0000) >> 16;
	buf[TCP_SEQACK_P+2] = 	(seq&0x0000ff00) >> 8;
	buf[TCP_SEQACK_P+3] = 	(seq&0x000000ff);
